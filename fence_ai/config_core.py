"""Extensible configuration manager for Fence AI.

Currently supports JSON and YAML files plus environment variables and runtime
overrides.  Designed to be easily extended: add a new Loader subclass and
register it via the ``register_loader`` decorator.  The precedence order is:

1. Defaults supplied to the class constructor (lowest)
2. Each *file* in the order provided
3. Environment variables with the given prefix (e.g. ``AWS_``)
4. Runtime overrides passed to :meth:`Config.merge`
...
def register_loader(cls):  # noqa: D401
    ...
    # Runtime structural check with Protocol can be tricky; rely on duck typing.
